...
// and constructor calls, e.g: "new CXXCLIGCNewExpr(foo)".
class CXXCLIGCNewExpr : public Expr {
...
public:
  ...
  CXXCLIGCNewExpr(ASTContext &C, CXXNewExpr::InitializationStyle initStyle,
             Expr *initializer, QualType ty, TypeSourceInfo *AllocatedTypeInfo,
             SourceLocation startLoc, SourceRange directInitRange);

  explicit CXXCLIGCNewExpr(EmptyShell Shell)
    : Expr(CXXCLIGCNewExprClass, Shell), Initializer(0) { }
  ...
  static bool classof(const Stmt *T) {
    return T->getStmtClass() == CXXCLIGCNewExprClass;
  }
  static bool classof(const CXXCLIGCNewExpr *) { return true; }
};
